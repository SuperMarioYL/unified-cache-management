...
- Benchmark 当前仅保留“待补充”占位。各使用指南保留必要验证，但不声称占位页已经提供评测方法。
...
安装器读取本版本根目录的 `release-manifest.json`。PyPI 发布完成时使用对应 index；未发布时直接安装清单中的 backend Wheel URL，不假定存在 PyPI 包：

| 构建类型 | 清单来源 |
| --- | --- |
| Tag / Stable | 同仓库、对应 Git 标签的完整 Schema 9 Release；支持正式版和 RC |
| Latest / PR | 同仓库最高版本、已完成且具有有效 Schema 9 清单的 Stable Release |
