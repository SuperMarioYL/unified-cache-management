...
!!! note "Migrated reference and validation scope"
    ...
    This guide preserves the [original repository reference](https://github.com/ModelEngine-Group/unified-cache-management/blob/a336d69bc03a550d44bee3df9da7664e9edfe3a7/docs/source/user-guide/prefix-cache/compress_store.md).
...
1. Replace `UCM_CONFIG_FILE` with the actual configuration file path.
2. Set `--tensor-parallel-size` for the target hardware.
3. Set `--max-model-len` to the model's supported context length.
