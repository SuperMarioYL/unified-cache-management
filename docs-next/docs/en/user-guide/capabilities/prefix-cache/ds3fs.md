...
The transfer workers create 3FS I/O vectors and rings, then register file
descriptors with 3FS. If you only need filesystem I/O through an existing mount,
...
Use [Installation](../../installation.md) for published engine artifacts and
[Build from source](../../../developer-guide/build_from_source.md) when the
...
`stream_number` controls Ds3fs transfer workers and defaults to 32; the sample
starts with 4 workers. Cache device-transfer concurrency is configured
...
Use the [vLLM quickstart](../../quick_start/quickstart_vllm.md) or its
[Ascend counterpart](../../quick_start/quickstart_vllm_ascend.md), pointing
...
4. Measure TTFT and throughput only after that read/write check passes, using
   the [benchmark procedure](../../../benchmark/index.md).
