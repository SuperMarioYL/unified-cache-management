...
The bundled profiles deliberately show complete deployment shapes, but they
carry values from one example cluster. Your `values-site.yaml` must replace:
...
| Model mount | Replace the root `extraStorage` NFS example, or use a supported `hostPath`, PVC, CSI, or NFS source. |
...
`storage.unifiedcacheStorage` and at least one `ucm_connector`.
