...
Select **Helm** in the [Quickstart installation selector](../../quick_start/index.md) and copy the Chart address from the release manifest. CUDA can use the packaged default image. For another version or Ascend, choose a complete address from the image comments in the packaged `values.yaml`.

=== "Release package"

    Replace the placeholder with the complete Chart download URL from the selector. The package URL already identifies the version.

    ```bash
    export UCM_CHART_URL='replace-with-chart-download-url'
    helm pull "$UCM_CHART_URL" --untar
