...
The 32 GiB threshold is an example, not a model-memory requirement. Choose thresholds from the deployment budget, then run:

```bash
ucm-toolkit run precheck --config /path/to/precheck.json --quick
```
