...
Root fields configure engine integration; fields inside `ucm_connector_config` configure the selected Store. The 4 GiB value is an explicit example budget. Configure only stages in use; backend guides describe other settings.
