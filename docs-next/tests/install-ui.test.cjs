...
test("Schema 8 Wheel shows the PyPI extra contract in official and fork docs", () => {
  ...
  const fork = JSON.parse(
    JSON.stringify(manifest)
      .replaceAll("uc-manager", "supermarioyl-uc-manager")
      .replaceAll("uc_manager", "supermarioyl_uc_manager")
      .replaceAll("https://pypi.org/", "https://test.pypi.org/")
  );
  Manifest.validateManifest(fork);
  const forkCuda = Selector.deriveSelection(Selector.buildSelectorModel(fork), {
    method: "wheel",
    engine: "vllm",
  });
  assert.equal(
