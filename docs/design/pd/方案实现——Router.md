...
`d0:9000` 只是 D 历史访问服务公布地址的示例，9000 不是约定默认端口，也不是 HTTP 推理端口或父类 KV 侧通道端口。这个历史访问服务需要在 D 的 UCM 实现中新增，其主机、端口和实际 engine ID 可在启动时公布。P 根据这些字段握手，再取得真实 rank、block 和设备地址；传输 payload 继续由 Transfer Engine 完成。
