...
- PEP 691 Simple API 支持 major version `1`，所有合法 `1.x` minor（包括更新 minor）均接受，不维护 fixed current-minor allowlist，也不把 warning 冻结为行为契约；unsupported major 或 malformed `meta.api-version` 是 protocol 硬失败，不能降级为 `binary-wheel-unavailable`。project response 只要求 `meta/name/files`，file 只要求 `filename/url/hashes`；`requires-python` 可选，unknown meta/project/file extension keys 忽略。本 resolver 不要求 PEP 700 `versions`、size 或 v1.1 字段。
