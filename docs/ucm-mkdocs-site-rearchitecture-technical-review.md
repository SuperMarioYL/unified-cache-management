...
- 不修改 UCM 运行时、推理引擎、Kthena、Helm Chart 或 PyMotor 的产品行为。
...
| 迁移成本 | 保持现状成本最低 | 需要转换导航、少量 MyST 结构和 URL | 采用旁路目录控制迁移风险 |
...
页面布局参考 vLLM 和 vLLM Ascend：顶部提供主导航、搜索、语言、版本和主题入口，桌面端使用左侧章节树、中间正文和右侧页内目录，移动端优先展示正文。安装选择器、兼容矩阵和 Benchmark 表格可以增加交互筛选，但必须保留可搜索的静态内容。

#### 4.7.2 AI 中文自动生成

英文是主内容来源，中文目录保持同路径镜像。作者修改英文文档后，AI Robot 识别新增或变化的页面，按照项目术语和写作约定生成中文，并把结果更新到同一个 PR。
...
| User Guide | Installation | 按 UCM 版本、引擎、设备、操作系统、架构和安装方式生成安装结果 |  |
...
| User Guide | Deploy | Docker、Kubernetes with Helm and Kthena、PyMotor |  |
