...
> 完整配置见 **[doc/uc-stack-kthena-values.md](doc/uc-stack-kthena-values.md)**（kthena 原生字段见 [doc/kthena-native-pd-multinode.md](doc/kthena-native-pd-multinode.md)）。
...
> PD 分离（prefill/decode + Mooncake/NIXL + 可选 UCM MultiConnector）的写法见 [doc/uc-stack-kthena-values.md](doc/uc-stack-kthena-values.md)；按需给某模型加 `modelSpec.pd` + prefill/decode 两 role。

📖 配置详解：**[doc/uc-stack-kthena-values.md](doc/uc-stack-kthena-values.md)**。本地校验渲染：`helm template <rel> . -f values.yaml -f models/<chip>/<file>.yaml`。
...
> 完整路由写法见模型示例与 [doc/uc-stack-kthena-values.md](doc/uc-stack-kthena-values.md)。
