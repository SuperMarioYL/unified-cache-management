...
def test_latest_admissible_candidate_is_selected_per_product_variant() -> None:
    ...
    assert [
        ...
        ("vllm-ascend", "a2", "v0.22.2rc1"),
        ("vllm-ascend", "a3", "v0.22.2rc1-a3"),
    ]
...
def test_superseded_compatible_candidates_are_each_excluded_once() -> None:
    ...
    assert superseded == [
        ...
        ("vllm-ascend", "v0.22.1rc3"),
        ("vllm-ascend", "v0.22.1rc3-a3"),
    ]


def test_latest_compatible_candidate_is_not_filtered_by_patch_metadata() -> None:
    ...
    ] == [("a2", "v0.22.2rc1"), ("a3", "v0.22.2rc1-a3")]
    assert all("runtime-patch" not in item["reason"] for item in plan["exclusions"])
...
def test_inspected_variant_mismatch_falls_back_within_canonical_group(
    ...
    snapshots = fixture["repositories"][repository]["snapshots"]
    older_a2 = copy.deepcopy(snapshots["v0.22.2rc1"])
    older_a2.update(
        {
            "upstream_tag": "v0.22.1rc3",
    ...
    ] == [("a2", "v0.22.1rc3"), ("a3", "v0.22.2rc1-a3")]
    assert {
...
def test_candidate_exclusion_reports_compatibility_unsupported_for_all_arches() -> None:
    ...
    assert (
        core.candidate_exclusion_reason(catalog, product, _vllm_candidate(product))
...
def test_release_plan_relaxed_accepts_non_pep440_version_without_patch_gate() -> None:
    catalog = _resolved_catalog(_catalog_for_plan())
    ...
    assert {task["runtime"]["version"] for task in plan.image_tasks} == {"latest"}
...
def test_pinned_tag_is_admitted_without_runtime_patch_metadata(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    assert [item["tag"] for item in plan["resolved_upstreams"]] == ["v0.22.2rc1-a3"]
    assert plan["exclusions"] == []
    assert len(plan["wheel_tasks"]) == 2
    assert len(plan["image_tasks"]) == 2
    assert len(plan["family_tasks"]) == 1
    registry.validate_resolved_plan(plan)
