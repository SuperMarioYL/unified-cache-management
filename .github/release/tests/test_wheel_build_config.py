...
def test_setup_accepts_authority_only_when_it_matches_version_ini(
    tmp_path: Path,
) -> None:
    config = _valid_config(cpu_arch=_host_architecture())
    config["authority"]["wheel_version"] = core.read_version()
    ...
    completed = subprocess.run(
        ...
        cwd=ROOT,
        env={**os.environ, "UCM_BUILD_CONFIG": str(config_path)},
        text=True,
        capture_output=True,
        check=True,
    )

    assert completed.stdout.splitlines()[-1] == core.read_version()


def test_setup_rejects_authority_version_drift(tmp_path: Path) -> None:
    config = _valid_config(cpu_arch=_host_architecture())
    config_path = tmp_path / "wheel-build.json"
    _write(config_path, config)
    ...
    assert "authority wheel_version differs from source version.ini" in completed.stderr
