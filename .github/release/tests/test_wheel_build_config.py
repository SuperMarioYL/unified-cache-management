    ...
    *, profile: str = "cann900-a2", cpu_arch: str = "amd64"
    ...
    task: dict[str, Any] = {
        ...
        **declaration,
        ...
        "runtime_patch_manifest": patch_manifest,
        "runtime_patch_manifest_sha256": core.runtime_patch_manifest_sha256(
            patch_manifest
        ),
        "write_authority": [],
        "build_eligible": True,
...
def _run_copied_setup(
    tmp_path: Path,
    config: dict[str, Any] | None,
    *,
    extra_env: dict[str, str] | None = None,
) -> subprocess.CompletedProcess[str]:
    source = _setup_source_copy(tmp_path)
    env = {
        key: value
        for key, value in os.environ.items()
        if key != "UCM_BUILD_CONFIG"
        and not key.startswith("UCM_RELEASE_")
        and key not in {"PLATFORM", "SOURCE_DATE_EPOCH", "UCM_DIST_NAME"}
    }
...
def test_setup_release_mode_uses_only_the_extended_v1_build_config(
    tmp_path: Path, profile: str, distribution: str, platform_arg: str
) -> None:
    task = _extended_task(profile=profile, cpu_arch=_host_architecture())
    config = {
        "authority": _extended_authority(task),
    ...
    result = _run_copied_setup(
        ...
        extra_env={
            ...
            "UCM_RELEASE_BUILD": "1",
            "UCM_RELEASE_PROFILE": "ignored",
        },
    )

    assert result.returncode == 0, result.stderr
    assert result.stdout.splitlines()[-2:] == [distribution, task["wheel_version"]]


def test_setup_without_build_config_preserves_ordinary_build_inputs(
    tmp_path: Path,
) -> None:
    result = _run_copied_setup(
        tmp_path,
        None,
        extra_env={"PLATFORM": "simu", "UCM_DIST_NAME": "uc-manager-local"},
    )

    assert result.returncode == 0, result.stderr
    assert result.stdout.splitlines()[-2:] == ["uc-manager-local", "0.6.0"]


def test_setup_rejects_legacy_release_mode_without_build_config(
    tmp_path: Path,
) -> None:
    result = _run_copied_setup(
        tmp_path,
        None,
        extra_env={"UCM_RELEASE_BUILD": "1", "PLATFORM": "ascend"},
    )

    assert result.returncode != 0
    assert "UCM_BUILD_CONFIG" in result.stderr


@pytest.mark.parametrize(
    ("extra_env", "message"),
    [
        ({"UCM_ENABLE_MINDIE": "1", "UCM_CXX11_ABI": "0"}, "MindIE"),
        ({"ENABLE_SPARSE": "true"}, "ENABLE_SPARSE"),
    ],
)
def test_setup_build_config_preserves_release_feature_exclusions(
    ...
    result = _run_copied_setup(tmp_path, config, extra_env=extra_env)
