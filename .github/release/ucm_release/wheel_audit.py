...
def validate_external_library_closure(
    ...
    *,
    ...
    allowed_deferred_libraries: Sequence[str],
) -> dict[str, Any]:
    ...
    allowed_deferred = sorted(set(allowed_deferred_libraries))
    for soname in allowed_deferred:
    ...
    deferred_roots = sorted(set(allowed_deferred) & set(roots))
    if deferred_roots:
        raise ValueError(f"direct external roots cannot be deferred: {deferred_roots}")
    ...
    unexpected_unresolved = sorted(set(unresolved_non_roots) - set(allowed_deferred))
    if unexpected_unresolved:
        raise ValueError(
            "unresolved external libraries are not explicitly deferred: "
            f"{unexpected_unresolved}"
        )
    return {
        "external_library_roots": roots,
        "external_libraries": sorted(external),
        "runtime_deferred_libraries": allowed_deferred,
