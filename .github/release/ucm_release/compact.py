...
def resolve_plan(
    ...
    *,
    ...
    patch_manifest = core.runtime_patch_manifest(bound)
    ...
    for upstream in upstreams:
        ...
        variant_config = next(
            item for item in product["variants"] if item["id"] == upstream["variant"]
        )
        patch_rule = core._matching_runtime_patch_rule(
            patch_manifest,
            {**upstream, "runtime_product": product["runtime_product"]},
            variant_config["runtime_patch_variants"][product["runtime_product"]],
            relaxed=relaxed,
        )
            ...
            images.append(
                {
                    ...
                    "runtime_patch_rule": patch_rule["id"],
                    "runtime_patch_variants": copy.deepcopy(
                        variant_config["runtime_patch_variants"]
                    ),
                    "runtime": {
                        key: upstream[key]
                        for key in (
